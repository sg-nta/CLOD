...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None, pre_attn=None):
        ...
        # pre_attn과 현재 attention weights를 합치고 평균내기
        if pre_attn is not None:
            pre_attn = pre_attn.view(N, Len_q, self.n_heads, self.n_levels * self.n_points)
            if attention_weights.shape == pre_attn.shape:
                attention_weights = (attention_weights + pre_attn) / 2
            else:
